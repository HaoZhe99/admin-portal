  ...
  const fetchProductDetail = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const productQuantity = get(product, ["quantity"], "");
        const productCategory = get(product, ["category"], "");
        ...
        setCategoryValue(productCategory);
        setDescriptionValue(productDescription);
      } else {
  ...
  const onClickSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const product = await res.json();
        toast(`Product "${product.name}" edited successfully!`);
      } else {
        const error = await res.json();
      ...
      toast("Error editing product:", error);
    } finally {
        ...
        <div className="p-6">
          ...
          {loading ? (
            ...
            <form onSubmit={onClickSubmit}>
              ...
              >
                {editLoading ? (
                  <LoadingOverlay loading={editLoading} />
                ) : (
                  <span>Submit</span>
                )}
              </button>
